...
listener_groups = ["all", "can", "eng"]          # all-listeners, C-E, English
dims            = ["2d", "3d"]

coords = {
    d: { s: { g: load_coords(s, g, int(d[:-1]))
...
def audio_for(row, stim):
    """return a list of wav files for a row"""
    if stim == "mixed":
...
def build_figure(df, dim, stim):
    df = df.copy()
    df["label"] = (df["speaker"] + "_" +
                   (df["language"].str.capitalize()
                    if stim != "mixed" else "Mixed"))
    cm  = color_map(df["speaker"])
    size = 12 if dim == "2d" else 6
    traces = []
    ...
    if dim == "2d":
        layout.update(xaxis=dict(title="Dimension 1", range=[-0.8, 0.8]),
...
app.layout = html.Div([
    ...
    html.P("Click a point to hear example audio"),

    # dimensionality
    dcc.Tabs(id="dim-tabs", value="2d", children=[
        dcc.Tab(label="2-D MDS", value="2d"),
        dcc.Tab(label="3-D MDS", value="3d")
    ], style={"width": "600px", "margin": "20px auto"}),

    # stimulus-set tabs are generated dynamically from dim
    html.Div(id="stim-tabs-wrapper"),

    # listener-group tabs are generated from dim × stim
...
def make_listener_tabs(dim, stim):
    ...
    return dcc.Tabs(children=[
        dcc.Tab(label="All listeners",              children=[dcc.Graph(id="g-all", figure=fig("all"))]),
        dcc.Tab(label="Cantonese-English listeners", children=[dcc.Graph(id="g-can", figure=fig("can"))]),
        dcc.Tab(label="English listeners",           children=[dcc.Graph(id="g-eng", figure=fig("eng"))])
...
def play_audio(cd_all, cd_can, cd_eng):
    click = next((c for c in (cd_all, cd_can, cd_eng) if c and c.get("points")), None)
    ...
    return html.Div(players or html.Div("Audio files not found"))
